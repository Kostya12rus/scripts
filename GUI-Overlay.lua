...
Overlay.Locale = {
	...
	["showcourier"] = {
		["english"] = "Show enemies courier items",
		["russian"] = "Показывать предметы вражеского курьера",
		["chinese"] = "向敵人展示快遞物品"
	},
	["width"] = {
		["english"] = 240,
...
function Overlay.OnDraw()
	...
	if not GUI.Exist(Overlay.Identity) then
		...
		GUI.AddMenuItem(Overlay.Identity, Overlay.Identity .. "showcourier", Overlay.Locale["showcourier"], GUI.MenuType.CheckBox, 0)
	...
	local showcourier = GUI.IsEnabled(Overlay.Identity .. "showcourier")
	...
	local heroes = {}
	for k, hero in pairs( (showcourier and NPCs.GetAll() or Heroes.GetAll()) ) do
		if hero ~= nil and not NPC.IsIllusion(hero) and (Entity.IsHero(hero) or NPC.IsCourier(hero)) and (showally or not Entity.IsSameTeam(myHero, hero)) then
			local hname = NPC.GetUnitName(hero)
			if	(hname ~= "npc_dota_hero_meepo" or heroes[hname] == nil or
				(Overlay.ItemsCount(hero) > Overlay.ItemsCount(hero))) then
				heroes[hname] = hero
			end
		end
	end
	...
	for k, hero in pairs(heroes) do
		...
		if showlastposition then 
			...
			then
				...
				if visible then
					...
					Renderer.DrawImage(GUIDB.Image('mini_' .. k), x - math.ceil(mapsize / 2), y - math.ceil(mapsize / 2), mapsize, mapsize)
					...
					if Overlay.Times[k] == nil then
						Overlay.Times[k] = os.clock()
					end
					...
					if timepass then
						...
						local seconds = math.floor((os.clock() - Overlay.Times[k])*10)/10
					...
					if drawpath then
						...
						direction:SetZ(0)
						direction:Normalize()
						direction:Scale(math.floor(speed * (os.clock() - Overlay.Times[k])))
						local pos = Entity.GetAbsOrigin(hero)
						...
						if vis then
							...
							Renderer.DrawImage(GUIDB.Image('mini_' .. k), xp - math.ceil(specsize / 2), yp - math.ceil(specsize / 2), specsize, specsize)
						end
					end
				end
			else
		...
		local heroImageHandle = GUIDB.Image(k)
		...
		for i = 0, (showextrainv and 8 or 5) do
			...
			if item ~= nil and Entity.IsAbility(item) and (not showonlyselected or hasValue(notify_items, Ability.GetName(item))) then
				...
				if (itemname == "item_power_treads") then
					if PowerTreads.GetStat(item) == 0 then
						itemname = "power_treads_str"
					end
					if PowerTreads.GetStat(item) == 1 then
						itemname = "power_treads_agi"
					end
					if PowerTreads.GetStat(item) == 2 then
						itemname = "power_treads_int"
					end
				end
				if (itemname == "item_bottle") then
					...
					if Item.GetCurrentCharges(item) == 1 then
						itemname = "bottle_small"
					end
					if Item.GetCurrentCharges(item) == 2 then
						itemname = "bottle_medium"
					end
					if Bottle.GetRuneType(item) == 0 then
						itemname = "bottle_doubledamage"
					end
					if Bottle.GetRuneType(item) == 1 then
						itemname = "bottle_haste"
					end
					if Bottle.GetRuneType(item) == 2 then
						itemname = "bottle_illusion"
					end
					if Bottle.GetRuneType(item) == 3 then
						itemname = "bottle_invisibility"
					end
					if Bottle.GetRuneType(item) == 4 then
						itemname = "bottle_regeneration"
					end
					if Bottle.GetRuneType(item) == 5 then
						itemname = "bottle_bounty"
					end
					if Bottle.GetRuneType(item) == 6 then
						itemname = "bottle_arcane"
					end
				end
				if (itemname == "item_armlet") then
					if (Ability.GetToggleState(item)) then
						itemname = "armlet_active"
					end
				end
				local imageHandle = GUIDB.Image(itemname)

				if isnotify then
					Overlay.Notify(item, notify_items, itemname, title, width, tts, imageHandle, heroImageHandle)
				end
...
function Overlay.ItemsCount(unit)
	local count = 0
	for i = 0, 8 do
		if NPC.GetItemByIndex(unit, i) ~= nil then
			count = count + 1
		end
	end
	return count
