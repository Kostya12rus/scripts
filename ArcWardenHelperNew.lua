...
function ArcHelper.mainAttack()
	...
	local hex = NPC.GetItem(myHero,"item_sheepstick")
	local null = NPC.GetItem(myHero,"item_nullifier")
	...
	if target then
			...
			if null and Ability.IsReady(null) and not NPC.HasModifier(target, "modifier_item_nullifier_mute") and not NPC.IsStunned(target) then
				if not ArcHelper.clone or not Entity.IsAlive(ArcHelper.clone) then
					Ability.CastTarget(null,target) -- or not(NPC.IsEntityInRange(ArcHelper.clone, target, 400) and NPC.IsEntityInRange(myHero, target, 800))
				end
				if ArcHelper.clone  and Entity.IsAlive(ArcHelper.clone) and NPC.IsEntityInRange(ArcHelper.clone, target, 800) and NPC.IsEntityInRange(myHero, target, 800) then
					local clone_null = NPC.GetItem(ArcHelper.clone, "item_nullifier");
					if Ability.GetCooldownLength(clone_null)-Ability.GetCooldownTimeLeft(clone_null)>=0.1 then
	...
	if ArcHelper.cloneAttacking then
		...
		if ArcHelper.cloneAttackingTarget then
			...
			if null and Ability.IsReady(null) 
				and not NPC.HasModifier(ArcHelper.cloneAttackingTarget, "modifier_item_nullifier_mute") and 
				not NPC.HasModifier(ArcHelper.cloneAttackingTarget, "modifier_item_nullifier_mute") 
				and not NPC.IsStunned(ArcHelper.cloneAttackingTarget) then
				
				ArcHelper.cloneTick = GameRules.GetGameTime() + 0.1
				Ability.CastTarget(null,ArcHelper.cloneAttackingTarget)
				return
			end
			...
			if force and Ability.IsReady(force) and ArcHelper.isTarget(ArcHelper.clone,ArcHelper.cloneAttackingTarget, 1800) and not NPC.IsEntityInRange(ArcHelper.cloneAttackingTarget, ArcHelper.clone, 500) and ArcHelper.useHurrican then
				ArcHelper.cloneTick = GameRules.GetGameTime() + 0.1
...
function ArcHelper.DrawCloneSwitchMsg()
	...
	if pike and ArcHelper.useHurrican then
		Renderer.DrawTextCentered(ArcHelper.font, w / 2, h / 2 + 350, "Use Hurrican", 1)
	end
